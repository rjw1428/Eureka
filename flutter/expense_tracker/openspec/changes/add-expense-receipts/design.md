...
**The hook.** `ScaffoldMessenger.showSnackBar` returns a controller whose `closed` future resolves with a `SnackBarClosedReason`. `action` means undo was pressed; every other reason — timeout, swipe, or being cleared by a subsequent `clearSnackBars()` — means commit. `_removeExpense` in `expenses_screen.dart` currently fires the snackbar and returns without awaiting it, so it needs to await that future and branch.
